...
struct JournalView: View {
    ...
    private var sortedEntries: [JournalEntry] {
        store.journalEntries.sorted { $0.date > $1.date }
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showingAddSheet) {
                AddJournalEntrySheet(store: store)
            }
        }
    }
            ...
            VStack(alignment: .leading, spacing: AppTheme.spacingS) {
                HStack {
                    HStack(spacing: 6) {
                        ...
                        Text(entry.mood.rawValue.uppercased())
                    ...
                    .overlay(
                        Rectangle().stroke(moodColor.opacity(0.3), lineWidth: 1)
                    )
